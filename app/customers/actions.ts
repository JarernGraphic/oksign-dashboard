...
export async function createCustomerAction(_state: CustomerFormState, formData: FormData): Promise<CustomerFormState> {
  ...
  const { data: newCustomer, error } = await supabase.from('customers').insert({
    ...
    facebook_name: parsed.data.facebookName || null,
    tax_id: parsed.data.taxId || null,
    address: parsed.data.address || null,
    lead_source_id: parsed.data.leadSourceId || null,
    note: parsed.data.note || null,
    created_by: profile.id,
  }).select('id').single();
  if (error) return { error: error.message };
  redirect(newCustomer ? `/customers/${newCustomer.id}?created=1` : '/customers?created=1');
}

export async function updateCustomerAction(customerId: string, _state: CustomerFormState, formData: FormData): Promise<CustomerFormState> {
  const parsed = customerSchema.safeParse({
    name: formData.get('name'),
    customerType: formData.get('customerType'),
    phone: formData.get('phone'),
