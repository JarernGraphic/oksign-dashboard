...
declare
  target_year integer := case when target_type = 'CUSTOMER' then 0 else extract(year from now() at time zone 'Asia/Bangkok')::integer end;
  next_value integer;
begin
  ...
  values (target_organization_id, target_type, target_year, 1)
  ...
  return case target_type
    when 'CUSTOMER' then 'CUS-' || lpad(next_value::text, 6, '0')
    when 'JOB' then 'JOB-' || target_year::text || '-' || lpad(next_value::text, 4, '0')
    else 'QT-' || target_year::text || '-' || lpad(next_value::text, 4, '0')
  end;
end;
$$;
