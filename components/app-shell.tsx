...
import {
  Bell, Boxes, BriefcaseBusiness, ChevronDown, CircleDollarSign, FileText,
  LayoutDashboard, MessageSquareText, Paintbrush, Plus, Search, Settings, Truck, Users,
} from 'lucide-react';
...
const navigation = [
  ...
  { label: 'งานขาย', items: [
    ...
    { name: 'Brief / งานใหม่', icon: MessageSquareText, href: '/jobs/new' },
...
export function AppShell({ children, profile, active, createHref = '/jobs/new' }: { children: ReactNode; profile: CurrentProfile; active: string; createHref?: string }) {
  return <main className="app-shell">
    <aside className="sidebar">
      <Link href="/" className="brand"><div className="brand-mark">OK</div><div><strong>OKSIGN</strong><span>Dashboard</span></div></Link>
      <nav className="navigation" aria-label="เมนูหลัก">
        {navigation.map((section) => <div className="nav-section" key={section.label}><p>{section.label}</p>{section.items.map((item) => {
          ...
          return <Link className={`nav-item ${active === item.href.split('?')[0] ? 'active' : ''}`} href={item.href} key={item.name}><Icon size={18} /><span>{item.name}</span></Link>;
        })}</div>)}
      </nav>
    ...
    <section className="workspace">
      <header className="topbar"><div className="mobile-brand">OKSIGN</div><label className="global-search"><Search size={18} /><input aria-label="ค้นหาทั้งระบบ" placeholder="ค้นหาเลขที่งาน ลูกค้า เบอร์โทร..." /><kbd>⌘ K</kbd></label><div className="top-actions"><button className="icon-button" aria-label="การแจ้งเตือน"><Bell size={20} /></button><Link className="primary-button" href={createHref}><Plus size={18} />สร้างงานใหม่</Link></div></header>
